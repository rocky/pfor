...
parallel since the C<-z> switch is not used: first `sa` is tried; after
that completes `bonzo`, and after that completes `filesvr`.
...
`/usr/local/etc/cpu10.hosts` you might try:
...
To blast out `home-opcuser.tar` to the sites listed in file 
`cpu27.hosts` and log the results:
